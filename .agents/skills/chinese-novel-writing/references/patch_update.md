...
In v0.3.1, `pending_updates/` is a review queue. Patch files default to `status: "pending"` and should include `schema_version: "0.3.1"` when generated by the helper scripts.
...
Automatically mergeable in v0.3.1:
...
Rollback means reverting the applied patch effects manually, using timestamped backups, or using version control. v0.3.1 does not provide semantic rollback.
